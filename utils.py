...
def quarterly_rebalancing(returns_df, port_div_tickers, target_weights, initial_capital):
    quarterly_dates = returns_df.resample('QE').last().index #ending dates of each quarter
    values_df = pd.DataFrame(index = returns_df.index, columns = port_div_tickers) #storing each ETF value over time separetely to adjust weights later
    values_df.iloc[0] = initial_capital * target_weights

    for i in range(1, len(returns_df)):
        current_date = returns_df.index[i]
        values_df.iloc[i] = values_df.iloc[i-1] * (1 + returns_df[port_div_tickers].iloc[i]) #adjust each ETF value simultaneously
    ...
    portfolio_div_equity = values_df.sum(axis=1)
    return portfolio_div_equity